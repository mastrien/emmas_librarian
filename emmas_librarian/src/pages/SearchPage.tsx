  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    for (const dbId of selectedDbs) {
      if (customQueries[dbId]) {
        ...
        if (!tr || !tr.isValid) {
          setError(
            `A busca automática falhou ou é incompatível com a base ${dbId}. Use uma query customizada ou desative a base.`,
          );
          return;
        }
    ...
    setError(null);
    try {
      const res = await projectService.searchAndPersist(
        parseInt(id),
        finalQueries,
        limit,
        sortBy,
        astToHumanString(ast),
      );
      setSummary(res);
    } catch (err: unknown) {
    ...
    <div className="fade-in" style={{ maxWidth: '1000px', margin: '0 auto', paddingBottom: '4rem' }}>
      ...
      <form onSubmit={handleSearch} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <div className="card" style={{ padding: '2rem' }}>
          ...
          >
            <label style={{ fontWeight: 600, color: 'var(--text-heading)', fontSize: '1.1rem' }}>
              Construtor Visual (Árvore Lógica)
            </label>
          </div>
          <div
            style={{
            ...
            {selectedDbs.map((dbId) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div style={{ fontWeight: 600, color: 'var(--text-heading)', textTransform: 'capitalize' }}>
                      {dbName}
                    </div>
                    <button
                      type="button"
                          ...
                          <>
                            <div
                              style={{
                                ...
                                background: 'var(--bg-main)',
                                borderRadius: 'var(--radius-sm)',
                                color: tr.isValid ? 'var(--text-main)' : 'var(--text-muted)',
                                wordBreak: 'break-all',
                              }}
                            >
                              {tr.query || 'Vazio'}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
            ...
            <div>
              ...
              >
                ...
                <span
                  style={{
                    color: selectedDbs.includes('crossref') && limit > 1000 ? 'var(--color-danger)' : 'inherit',
                  }}
                >
                  • Crossref: máximo de 1.000 resultados.
                </span>
                <span
                  style={{ color: selectedDbs.includes('scopus') && limit > 5000 ? 'var(--color-danger)' : 'inherit' }}
                >
                  • Scopus: a API pode limitar entre 200 e 5.000 dependendo da assinatura institucional (pode falhar em
                  limites altos).
                </span>
                ...
                {limit > 200 && selectedDbs.includes('scopus') && (
                  <span style={{ color: 'var(--color-warning)', fontWeight: 600, marginTop: '0.25rem' }}>
                    Aviso: A base Scopus pode retornar erro (Exceeds maximum) para limites &gt 200 dependendo do seu
                    nível de serviço.
                  </span>
                )}
              </div>
      ...
      {summary && (
        <SearchSummaryModal isOpen={!!summary} onClose={() => navigate(`/projects/${id}`)} summary={summary} />
      )}
          ...
          >
            ...
            >
              ...
              <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', lineHeight: '1.5' }}>
                Para realizar buscas na <strong>{showKeyAlert.db === 'wos' ? 'Web of Science' : 'Scopus'}</strong>, você
                precisa primeiro configurar sua chave de API nas configurações do sistema.
              </p>
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button
                  onClick={() => setShowKeyAlert({ ...showKeyAlert, isOpen: false })}
                  className="btn-secondary"
                  style={{ flex: 1 }}
                >
                  Cancelar
