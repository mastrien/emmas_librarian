...
export const ArticleTable: React.FC<ArticleTableProps> = memo(({
  ...
  return (
    <div className="card" style={{ border: 'none', marginBottom: '2rem', height: '80vh', display: 'flex', flexDirection: 'column' }}>
      {activeArticlesLength === 0 ? (
        ...
        <TableVirtuoso
          ...
          components={{
            Table: (props) => <table {...props} style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '800px' }} />,
            TableRow: (props) => <tr {...props} style={{ borderBottom: '1px solid var(--border-color)', transition: 'background var(--transition-fast)' }} onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-main)'} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'} />
          }}
          fixedHeaderContent={() => (
              ...
              <td style={{ padding: '1.25rem 1.5rem' }}>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  {article.local_file_path ? (
                    <button onClick={() => onUnlinkClick(article.id)} className="btn-secondary" style={{ padding: '0.4rem 0.6rem', fontSize: '0.8rem', color: 'var(--color-danger)' }} title="Desvincular PDF">
                      <XIcon size={14} />
                    </button>
                  ) : (
                    <button onClick={() => onUploadClick(article.id)} disabled={uploadingId === article.id} className="btn-secondary" style={{ padding: '0.4rem 0.6rem', fontSize: '0.8rem' }} title="Vincular PDF Local">
                      {uploadingId === article.id ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />}
                    </button>
                  )}

                  <button onClick={() => onStatusChange(article.id, 'read')} className="btn-secondary" style={{ padding: '0.4rem 0.6rem', fontSize: '0.8rem' }} title="Marcar como Lido">
                    <CheckCircle size={14} /> Lido
                  </button>
                  
                  {isArticleManual(article) && (
                    <button onClick={() => onEditClick(article)} className="btn-secondary" style={{ padding: '0.4rem 0.6rem', fontSize: '0.8rem' }} title="Editar Metadados">
                      <Edit2 size={14} /> Editar
                    </button>
                  )}

                  <button onClick={() => onArchiveClick(article.id)} className="btn-secondary" style={{ padding: '0.4rem 0.6rem', fontSize: '0.8rem', color: 'var(--color-danger)' }} title="Arquivar">
