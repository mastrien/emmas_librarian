...
export default function QuestionSetCatalog({ 
  ...
  return (
    <div className="question-set-catalog">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        {isCreatingExternal === undefined && !isCreating && (
          <button 
            ...
            title={currentQuestions.filter(q => q.trim().length > 0).length === 0 ? "Adicione perguntas acima para salvar" : "Salvar perguntas atuais como novo conjunto"}
      ...
      {isCreating && (
        <form onSubmit={handleCreate} style={{ background: 'var(--bg-surface)', padding: '1rem', borderRadius: '4px', border: '1px solid var(--color-primary)', marginBottom: '1rem' }}>
          ...
          <div style={{ marginBottom: '0.8rem', fontSize: '0.85em', display: 'flex', gap: '1rem' }}>
            <label><input type="radio" checked={formScope === 'project'} onChange={() => setFormScope('project')} /> Neste Projeto</label>
            <label><input type="radio" checked={formScope === 'global'} onChange={() => setFormScope('global')} /> Global (Todos os Projetos)</label>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button type="submit" className="btn-primary" style={{ padding: '0.3rem 0.8rem', fontSize: '0.85em' }}>Salvar</button>
            <button type="button" className="btn-secondary" onClick={() => setIsCreating(false)} style={{ padding: '0.3rem 0.8rem', fontSize: '0.85em' }}>Cancelar</button>
          </div>
        </form>
      )}

      {sets.length === 0 && !isCreating ? (
        <div style={{ fontSize: '0.9em', color: '#666', padding: '1rem', border: '1px dashed #ccc', textAlign: 'center', borderRadius: '4px' }}>
          Nenhum conjunto de perguntas encontrado.<br/>
          Crie perguntas acima e clique em "+ Salvar Atual" para guardá-las.
        </div>
      ) : (
        <div className="catalog-list" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '300px', overflowY: 'auto' }}>
          {sets.map(set => {
            ...
            if (isEditingThis) {
              return (
                <div key={set.id} style={{ border: '1px solid var(--color-primary)', padding: '0.8rem', borderRadius: '4px', background: 'var(--bg-surface)' }}>
                  <input 
                    autoFocus
                    type="text" 
                    value={formName} 
                  ...
                  />
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button onClick={() => handleUpdate(set)} className="btn-primary" style={{ padding: '0.2rem 0.5rem' }}><Check size={14}/></button>
                    <button onClick={() => setEditingId(null)} className="btn-secondary" style={{ padding: '0.2rem 0.5rem' }}><X size={14}/></button>
                  </div>
                </div>
              );
            }
            ...
            return (
              <div key={set.id} className="catalog-item" style={{ border: '1px solid var(--border-color)', padding: '0.8rem', borderRadius: '4px', background: 'var(--bg-main)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <h4 style={{ margin: '0 0 0.2rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {set.name} 
                      <span style={{ fontSize: '0.7em', padding: '0.1rem 0.3rem', background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: '3px' }}>
                        {set.project_id === null ? 'Global' : 'Projeto'}
                      </span>
                    </h4>
                    {set.description && <p style={{ margin: '0 0 0.4rem 0', fontSize: '0.85em', color: 'var(--text-muted)' }}>{set.description}</p>}
                    <div style={{ fontSize: '0.75em', color: 'var(--text-muted)' }}>
                      {qCount} {qCount === 1 ? 'pergunta' : 'perguntas'} &middot; {new Date(set.updated_at || Date.now()).toLocaleDateString()}
                    </div>
                  </div>
                  
                  <div style={{ display: 'flex', gap: '0.3rem', alignItems: 'center' }}>
                    <button onClick={() => handleUse(set)} className="btn-primary" style={{ padding: '0.3rem 0.6rem', fontSize: '0.85em', marginRight: '0.5rem' }}>Usar</button>
                    <button onClick={() => startEdit(set)} title="Editar" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><Edit2 size={15} /></button>
                    <button onClick={() => handleDuplicate(set.id)} title="Duplicar" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><Copy size={15} /></button>
                    <button onClick={() => handleDelete(set.id)} title="Excluir" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--color-danger)' }}><Trash2 size={15} /></button>
                  </div>
                </div>
              </div>
            );
