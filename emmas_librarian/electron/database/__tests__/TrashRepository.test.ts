...
describe('TrashRepository', () => {
  ...
  describe('Trash Operations', () => {
    it('should get trash items from projects, articles, and annotations', () => {
      ...
      const aInfo = db
        .prepare("INSERT INTO articles (project_id, title, deleted_at) VALUES (?, 'A1', datetime('now'))")
        .run(pActiveId);
      const aId = aInfo.lastInsertRowid;

      const annInfo = db
        .prepare(
          "INSERT INTO annotations (article_id, content_markdown, deleted_at) VALUES (?, 'Ann1', datetime('now'))",
        )
        .run(aId);
    ...
    it('should delete article permanent without physical file', () => {
      ...
      expect(db.prepare('SELECT count(*) as c FROM annotations WHERE article_id = ?').get(aId) as any).toEqual({
        c: 0,
      });
    ...
    it('should delete article permanent with physical file', () => {
      const pInfo = db.prepare("INSERT INTO projects (name) VALUES ('P1')").run();
      const aInfo = db
        .prepare("INSERT INTO articles (project_id, title, local_file_path) VALUES (?, 'A1', '/path/to/pdf')")
        .run(pInfo.lastInsertRowid);
      const aId = aInfo.lastInsertRowid as number;
    ...
    it('should catch error when deleting article permanent physical file', () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining(`Failed to delete physical PDF for article ${aId}:`),
        expect.any(Error),
      );
      expect(db.prepare('SELECT count(*) as c FROM articles WHERE id = ?').get(aId) as any).toEqual({ c: 0 });
    });
    ...
    it('should delete annotation permanent', () => {
      ...
      const aInfo = db.prepare("INSERT INTO articles (project_id, title) VALUES (?, 'A1')").run(pInfo.lastInsertRowid);
      const annInfo = db
        .prepare("INSERT INTO annotations (article_id, content_markdown) VALUES (?, 'Ann1')")
        .run(aInfo.lastInsertRowid);
    ...
    it('should empty trash', () => {
      ...
      const aInfo = db
        .prepare("INSERT INTO articles (project_id, title, deleted_at) VALUES (?, 'A1', datetime('now'))")
        .run(pActiveInfo.lastInsertRowid);
      ...
      expect(db.prepare('SELECT count(*) as c FROM articles WHERE id = ?').get(aInfo.lastInsertRowid) as any).toEqual({
        c: 0,
      });
      expect(
        db.prepare('SELECT count(*) as c FROM annotations WHERE id = ?').get(annInfo.lastInsertRowid) as any,
      ).toEqual({ c: 0 });
    });
  });
});
