...
describe('ArticleRepository', () => {
  ...
  let categories: ArticleCategoryRepository;
  let pdfs: PdfLibraryRepository;
  ...
  beforeEach(() => {
    ...
    categories = new ArticleCategoryRepository(db);
    pdfs = new PdfLibraryRepository(db);

    const projectInfo = db
      .prepare('INSERT INTO projects (name, created_at) VALUES (?, ?)')
      .run('Test Project', new Date().toISOString());
  ...
  describe('Articles', () => {
    it('should save a new article', () => {
      const id = repo.saveArticle(projectId, {
        title: 'Test Article',
        doi: '10.123/456',
    ...
    it('should merge duplicate article when saving', () => {
      const id1 = repo.saveArticle(projectId, {
        title: 'Test Article',
        doi: '10.123/456',
        source_databases: '["pubmed"]',
      });
      const id2 = repo.saveArticle(projectId, {
        title: 'Test Article',
        doi: '10.123/456',
  ...
  describe('Article Categories', () => {
    ...
    beforeEach(() => {
      ...
      const res1 = db
        ...
        .run(projectId, 'TextCat', 'text');
      textCatId = Number(res1.lastInsertRowid);

      const res2 = db
        .prepare('INSERT INTO project_categories (project_id, name, type) VALUES (?, ?, ?)')
        .run(projectId, 'EnumCat', 'multiselect');
      enumCatId = Number(res2.lastInsertRowid);

      opt1Id = Number(
        db.prepare('INSERT INTO project_category_options (category_id, name) VALUES (?, ?)').run(enumCatId, 'Opt1')
          .lastInsertRowid,
      );
      opt2Id = Number(
        db.prepare('INSERT INTO project_category_options (category_id, name) VALUES (?, ?)').run(enumCatId, 'Opt2')
          .lastInsertRowid,
      );
    });

    it('should return empty if category not found in setArticleCategory', () => {
      expect(() => categories.setArticleCategory(articleId, 999, 'val')).not.toThrow();
    });

    it('should set text category', () => {
      categories.setArticleCategory(articleId, textCatId, 'My Value');
      let cats = categories.getArticleCategories(articleId);
      expect(cats).toHaveLength(1);
      expect(cats[0].value).toBe('My Value');

      // Update existing
      categories.setArticleCategory(articleId, textCatId, 'New Value');
      cats = categories.getArticleCategories(articleId);
      expect(cats[0].value).toBe('New Value');

      // Delete if null or empty
      categories.setArticleCategory(articleId, textCatId, '');
      cats = categories.getArticleCategories(articleId);
    ...
    it('should swallow errors when inserting invalid enum option', () => {
      ...
      categories.setArticleCategory(articleId, enumCatId, [opt1Id] as any);
      // Should not throw
    ...
    it('should get all project article categories', () => {
      categories.setArticleCategory(articleId, textCatId, 'TextVal');
      categories.setArticleCategory(articleId, enumCatId, [opt1Id] as any);

      const a2 = repo.saveArticle(projectId, { title: 'A2' });
      categories.setArticleCategory(a2, enumCatId, [opt2Id] as any);

      const all = categories.getAllProjectArticleCategories(projectId);
  ...
  describe('PDFs and Library', () => {
    it('should get stored pdfs', () => {
      ...
      const stored = pdfs.getStoredPdfs();
      expect(stored).toHaveLength(1);
      expect((stored[0] as any).articles).toHaveLength(1);
      expect((stored[0] as any).articles[0].article_title).toBe('A1');
    });

    it('should get articles for pdf', () => {
      const a1 = repo.saveArticle(projectId, { title: 'A1' } as any);
      repo.updateArticleFilePath(a1, '/test.pdf');
      const articles = pdfs.getArticlesForPdf('/test.pdf');
    ...
    it('should unlink pdf from article', () => {
      const a1 = repo.saveArticle(projectId, { title: 'A1' } as any);
      repo.updateArticleFilePath(a1, '/test.pdf');
      db.prepare('INSERT INTO pdf_chunks (article_id, chunk_index, page_number, text_content) VALUES (?, ?, ?, ?)').run(
        a1,
        0,
        1,
        'text',
    ...
    it('should handle unlinkPdfFromArticle gracefully if article has no local_file_path', () => {
      ...
      expect(() => pdfs.unlinkPdfFromArticle(a1)).not.toThrow();
    });
    ...
    it('should delete pdf library record and unlink', () => {
      ...
      const unlinkedIds = pdfs.deletePdfLibraryRecord('/test.pdf');
    ...
    it('should register pdf in library', () => {
      pdfs.registerPdfInLibrary('/reg.pdf', 'hash123', 'reg.pdf', 100);
    ...
    it('should get pdf by hash', () => {
      ...
      const pdf = pdfs.getPdfByHash('myhash');
      expect(pdf.file_path).toBe('/h.pdf');
    });

    it('should link pdf to article (exact match)', () => {
      const a1 = repo.saveArticle(projectId, { title: 'A1' } as any);
      pdfs.registerPdfInLibrary('/link.pdf', 'h1', 'link.pdf', 10);
      pdfs.linkPdfToArticle(a1, '/link.pdf');
    ...
    it('should link pdf to article (case insensitive match)', () => {
      const a1 = repo.saveArticle(projectId, { title: 'A1' });
      pdfs.registerPdfInLibrary('C:/MyFolder/link.pdf', 'h1', 'link.pdf', 10);
      pdfs.linkPdfToArticle(a1, 'c:/myfolder/link.pdf');
    ...
    it('should link pdf to article (filename match)', () => {
      const a1 = repo.saveArticle(projectId, { title: 'A1' } as any);
      pdfs.registerPdfInLibrary('/different/path/link.pdf', 'h1', 'link.pdf', 10);
      pdfs.linkPdfToArticle(a1, '/some/other/link.pdf');
    ...
    it('should throw when linking pdf that is not found', () => {
      const a1 = repo.saveArticle(projectId, { title: 'A1' });
      expect(() => pdfs.linkPdfToArticle(a1, '/not/found.pdf')).toThrow(/PDF file not found/);
    });
  ...
  describe('Clone/Import Articles', () => {
    it('should import articles from another project', () => {
      const destProject = Number(
        db.prepare('INSERT INTO projects (name, created_at) VALUES (?, ?)').run('Dest', new Date().toISOString())
          .lastInsertRowid,
      );
