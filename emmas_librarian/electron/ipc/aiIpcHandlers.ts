...
export function setupAiIpcHandlers(db: DatabaseAdapter, aiService: AIService, ipcMainModule: typeof ipcMain = ipcMain) {
  ...
  ipcMainModule.handle(IpcChannel.AI_GENERATE_SUMMARY, withErrorHandling(async (event, articleId: number) => {
  ...
  ipcMainModule.handle(IpcChannel.AI_MASSIVE_EXTRACTION, withErrorHandling(async (event, articleId: number, questions: string[]) => {
    const article = db.getArticle(articleId);
    if (!article || !article.local_file_path || !fs.existsSync(article.local_file_path)) {
      throw new Error('PDF not found for this article.');
    }
  ...
  ipcMainModule.handle(IpcChannel.AI_EXTRACT_METADATA, withErrorHandling(async (event, articleId: number) => {
    const article = db.getArticle(articleId);
    if (!article || !article.local_file_path || !fs.existsSync(article.local_file_path)) {
      throw new Error('PDF not found for this article.');
    }
  ...
  ipcMainModule.handle(IpcChannel.AI_MODEL_CONFIG_UPDATE, async (_, skill: AISkill, provider: AIProvider, modelName: string) => {
  ...
  ipcMainModule.handle(IpcChannel.INVESTIGATION_RESULTS_SAVE, async (_, investigationId: number, articleId: number, results: InvestigationResultInput[]) => {
  ...
  ipcMainModule.handle(IpcChannel.INVESTIGATION_RESULTS_GET_BY_ARTICLE, async (_, investigationId: number, articleId: number) => {
