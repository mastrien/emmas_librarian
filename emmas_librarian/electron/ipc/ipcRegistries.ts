...
export function setupIpcRegistries() {
    ...
    ipcMain.handle(IpcChannel.ARTICLES_CREATE_FROM_PDFS, async (event, projectId, filePaths) => {
        ...
        if (filePaths.length > 0) {
            try {
                searchId = db.saveSearchHistory(projectId, `Importação em Lote de ${filePaths.length} PDFs`, {}, filePaths.length, { Manual: { count: filePaths.length } });
            }
            catch (err) {
    ...
    ipcMain.handle(IpcChannel.MASSIVE_INVESTIGATIONS_SAVE, (event, projectId, questions, articlesIds, modelUsed, status) => {
