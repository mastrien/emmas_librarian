...
export class ApiIntegrator {
  ...
  async searchOpenAlex(filterStr: string): Promise<NormalizedArticle[]> {
    try {
      const url = new URL(this.OPENALEX_URL);
      if (filterStr) url.searchParams.append("filter", filterStr);
  ...
  async searchCrossref(params: Record<string, string>): Promise<NormalizedArticle[]> {
    try {
