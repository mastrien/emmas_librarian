...
export class QueryTranslator {
  ...
  // --- SCOPUS ---
  private translateToScopus(node: QueryASTNode): QueryTranslationResult {
    try {
  ...
  private scopusVisit(node: QueryASTNode): string {
    if (node.type === 'rule') {
  ...
  // --- Web of Science (WoS) ---
  private translateToWoS(node: QueryASTNode): QueryTranslationResult {
    try {
  ...
  private wosVisit(node: QueryASTNode): string {
    if (node.type === 'rule') {
      const fieldMap: Record<QueryField, string> = {
        all: 'TS',
        title: 'TI',
  ...
  private translateToOpenAlex(node: QueryASTNode): QueryTranslationResult {
    try {
  ...
  private openalexVisit(node: QueryASTNode): string {
    if (node.type === 'rule') {
      const fieldMap: Record<QueryField, string> = {
        all: 'default.search',
        title: 'title.search',
  ...
  private translateToCrossref(node: QueryASTNode): QueryTranslationResult {
    try {
  ...
  private crossrefVisit(node: QueryASTNode): string {
  ...
  private crossrefCollect(node: QueryASTNode): Record<string, string[]> {
